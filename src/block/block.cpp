...
std::vector<uint8_t> Block::encode() {
    ...
    size_t total_size =
        data.size() + entry_nums * sizeof(uint16_t) + sizeof(uint16_t); //用sizeof才是严谨的写法
...
std::shared_ptr<Block> Block::decode(const std::vector<uint8_t>& encoded, bool with_hash) {
    ...
    // 从最后两字节获得实体数量（小端）
    uint16_t entry_nums = encoded[end_pos - 2] | (static_cast<uint16_t>(encoded[end_pos - 1]) << 8);
    ...
    for (size_t i = 0; i < entry_nums; ++i) {
        ...
        uint16_t offset = encoded[pos] | (static_cast<uint16_t>(encoded[pos + 1]) << 8);
        if (offset >= data_end) {
...
std::string Block::get_last_key(){
    ...
    return get_key_at(offsets.back());
}
...
std::string Block::get_key_at(size_t offset) const {
    ...
    uint16_t key_len = data[offset] | (static_cast<uint16_t>(data[offset + 1]) << 8);
    size_t key_pos = offset + 2;
    if (key_pos + key_len > data.size()) {
        throw std::runtime_error("invalid key length in block");
    }

    return std::string(reinterpret_cast<const char*>(data.data() + key_pos), key_len);
}
...
std::string Block::get_value_at(size_t offset) const {
    ...
    uint16_t value_len =
        data[value_len_pos] | (static_cast<uint16_t>(data[value_len_pos + 1]) << 8);
    if (value_len_pos + 2 + value_len > data.size()) {
        throw std::runtime_error("invalid value length in block");
    }

    return std::string(reinterpret_cast<const char*>(data.data() + value_len_pos + 2), value_len);
}
...
int Block::compare_key_at(size_t offset, const std::string& target) const {
    ...
    std::string_view key(reinterpret_cast<const char*>(data.data() + key_pos), key_len);
...
int Block::adjust_idx_by_tranc_id(size_t idx, uint64_t tranc_id) {
    ...
    for (size_t i = idx; i < offsets.size() && get_key_at(offsets[i]) == target_key; ++i) {
...
Block::iters_preffix(uint64_t tranc_id, const std::string& preffix) {
    ...
    return get_monotony_predicate_iters(tranc_id, [&preffix](const std::string& key) -> int {
...
Block::get_monotony_predicate_iters(uint64_t tranc_id,
    ...
    auto begin_ptr = std::make_shared<BlockIterator>(shared_from_this(), first_pos, tranc_id);
    auto end_ptr = std::make_shared<BlockIterator>(shared_from_this(), last_pos + 1, tranc_id);
    return std::make_optional(std::make_pair(std::move(begin_ptr), std::move(end_ptr)));
    // return
