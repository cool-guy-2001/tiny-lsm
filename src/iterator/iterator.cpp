...
bool operator>(const SearchItem& a, const SearchItem& b) {
    ...
    if (a.tranc_id_ != b.tranc_id_) {
        return a.tranc_id_ < b.tranc_id_; //只要tranc_id不等，那么tranc_id大的就先出堆
    }
    if (a.level_ != b.level_) {
...
HeapIterator::HeapIterator(std::vector<SearchItem> item_vec, uint64_t max_tranc_id,
                           bool skip_delete, bool keep_all_versions)
