...
MemTable::get_batch(const std::vector<std::string>& keys, uint64_t tranc_id) {
    ...
    if (!std::any_of(results.begin(), results.end(),
                     [](const auto& result) { return !result.second.has_value(); })) {
        return results;
    }
    ...
    for (size_t idx = 0; idx < keys.size(); idx++) {
        ...
        if (frozen_result.is_valid()) {
            ...
            results[idx] = std::make_pair(
                key, std::make_pair(frozen_result.get_value(), frozen_result.get_tranc_id()));
        } else {
...
HeapIterator MemTable::iters_preffix(const std::string& preffix, uint64_t tranc_id) {
    ...
    for (auto it = frozen_tables.rbegin(); it != frozen_tables.rend(); ++it) {
        ...
        while (it1.is_valid() && has_prefix(it1.get_key(), preffix)) {
            item_vec.push_back(
                SearchItem(it1.get_key(), it1.get_value(), idx, 0, it1.get_tranc_id()));
            ++it1;
        }
        idx++;
    }
    if (current_table) {
        ...
        while (tmp_it.is_valid() && has_prefix(tmp_it.get_key(), preffix)) {
            item_vec.push_back(
                SearchItem(tmp_it.get_key(), tmp_it.get_value(), idx, 0, tmp_it.get_tranc_id()));
            ++tmp_it;
        }
    }
...
MemTable::iters_monotony_predicate(uint64_t tranc_id,
